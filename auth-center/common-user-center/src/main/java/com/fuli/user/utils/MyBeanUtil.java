...
//							log.debug("字符串时间格式化失败  格式为： " + YYYY_MM_DD_T_HH_MM_SS);
							try {
								date = formatter2.parse(strValue);
							} catch (Exception e1) {
//								log.debug("字符串时间格式化失败  格式为： " + YYYY_MM_DD_HH_MM_SS);
	...
	private static <T> Collection handleCollection(Type fGenericType, Collection colProperties,
			String... ormPackageNames) throws Exception {
